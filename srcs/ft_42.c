...
/*   Updated: 2016/01/27 22:27:43 by amoinier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
void			update_draw(t_env *init, t_point ***tab)
{
	int	i;
	int	j;
	int	ij[2];
	...
	while (i < tab[0][0]->sizeline)
	{
		...
		{
			tab[i][j]->px = init->movex + ((tab[i][j]->x * init->zoom));
			tab[i][j]->px = ((tab[i][j]->px - tab[i][tab[0][0]->sizecol / 2]->px) * cos(init->axex) + tab[i][tab[0][0]->sizecol / 2]->px - (tab[i][j]->py - tab[i][tab[0][0]->sizecol / 2]->py) * sin(init->axex));
